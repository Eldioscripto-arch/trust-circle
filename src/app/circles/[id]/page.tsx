...
export default function CircleDetailPage() {
  ...
  useEffect(() => {
    ...
    if (txId && txFailed) {
      setError('La transacción falló. Intentá de nuevo.')
      setContributing(false)
    }
  }, [txConfirmed, txFailed, txId])
  ...
  if (loading) return (
    <div style={{ background: '#0d1117', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ color: '#f0b429', fontSize: 18 }}>Cargando...</div>
    </div>
  )
  ...
  if (!circle) return (
    <div style={{ background: '#0d1117', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      <div style={{ color: '#e53e3e', fontSize: 18 }}>Círculo no encontrado</div>
  ...
  return (
    <div style={{ background: '#0d1117', minHeight: '100vh', maxWidth: 430, margin: '0 auto', color: '#e2e8f0', fontFamily: "'DM Sans',sans-serif", paddingBottom: 120 }}>

      <div style={{ padding: '20px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <button onClick={() => router.push('/')} style={{ background: 'none', border: 'none', color: '#f0b429', fontSize: 22, cursor: 'pointer' }}>←</button>
      ...
      <div style={{ margin: '16px 20px 0', background: '#161b22', border: '1px solid #2a3441', borderRadius: 20, padding: 20 }}>
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <div style={{ height: 6, background: '#1c2330', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ height: '100%', borderRadius: 3, width: `${progressPct}%`, background: slotsLeft > 0 ? '#f0b429' : '#68d391' }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '16px 20px 0' }}>
        {!isMember && canJoin && (
          <button onClick={handleJoin} disabled={joining}
            style={{ width: '100%', background: joining ? '#a07820' : 'linear-gradient(135deg,#f0b429,#ed8936)', color: '#000', border: 'none', padding: 16, borderRadius: 16, fontSize: 16, fontWeight: 700, cursor: joining ? 'not-allowed' : 'pointer' }}>
